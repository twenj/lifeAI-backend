...
Default API address: `http://0.0.0.0:8787`.

The uni-app frontend now uses this backend for both H5 and the WeChat development simulator. H5 proxies `/backend/*` to this service; the WeChat development simulator uses `http://0.0.0.0:8787` directly. A deployed mini-program needs an HTTPS API domain added to its request whitelist and a production API base URL in `uniapp/src/lib/api.js`.
